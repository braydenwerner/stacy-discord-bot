...
bot_env_path = env_path.replace("instance.env", "bot.env")
try:
    budget_name = stack_out("BudgetName")
except subprocess.CalledProcessError:
    budget_name = "stacy-mc-credits"
bot_lines = [
    f"# Generated by deploy.sh from stack {stack}",
    f"AWS_REGION={aws_region}",
    f"AWS_ACCESS_KEY_ID={stack_out('BotAccessKeyId')}",
    f"AWS_SECRET_ACCESS_KEY={stack_out('BotSecretAccessKey')}",
    f"MINECRAFT_INSTANCE_ID={instance_id}",
    f"MINECRAFT_SERVER_HOST={mc_host}",
    f"MINECRAFT_PORT={mc_port}",
    f"MINECRAFT_BACKUP_BUCKET={stack_out('BackupBucket')}",
    f"AWS_BUDGET_NAME={budget_name}",
    f"AWS_PROMO_CREDIT_USD={params.get('MonthlyBudgetUsd') or '1000'}",
    f"AWS_ACCOUNT_ID={account_id}",
    "",
]
open(bot_env_path, "w").write("\n".join(bot_lines))
import os
os.chmod(bot_env_path, 0o600)
print(f"Wrote {bot_env_path} — copy to the Pi and run: pnpm run minecraft:sync-env")
PY
