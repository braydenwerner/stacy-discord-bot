...
const TOOL_DIRECTIVE =
  " Actions always work no matter your mood. When the user asks to play, pause, resume, skip, view the queue, or get lyrics, call the matching music tool. When the user explicitly asks to send, tell, relay, or deliver a message to one or more known people (e.g. \"send ben this\", \"tell michael that\"), call the sendMessage tool — only when they actually ask to message someone, never just because a name is mentioned. For contacts (Equality role only): manageContact to add/remove/update a person (name + Discord user ID; both required for add); listContacts (or /people) to show all known people. For user groups (Equality role only): createGroup to create a group and add multiple people in one call (use \"me\" for the requester); manageUserGroup only for a single add/remove or delete; listUserGroups to show groups; pingGroup to @mention every member with a message. For nice vs snarky tone (bot owner only): manageToneList with nice_add, nice_remove (snarky), or list — only when the owner asks to change who gets which tone. When the user explicitly asks to open, create, or make a pull request/PR to this repo (e.g. \"open a PR to add X\"), call the openPullRequest tool with a full description of the change as the task. You may word your reply however your persona dictates, but never refuse or sabotage the action itself.";
